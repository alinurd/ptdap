<script>
    ...
    function actionMultiDeleteItems() {
        ...
        .then((result) => { if (result.isConfirmed) { swAlertDialog('success', 'Berhasil menghapus'); $.get("{{ route($prefixRoute.'multi_delete') }}", { id: id }, () => location.reload()); } });
    }
        ...
        $.get(url, function(res) {
            if (res.status == 'success') {
                $.each(res.data[0], function(name, val) {
                    ...
                    if (name === 'icon' && val) $('#formData #holder img').attr('src', "{{ url('/') }}/" + val);
                });
                $('#data_id').val(id);
