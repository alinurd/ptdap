        ...
        <x-table-btn>
            ...
            <x-slot name="tbody">
                ...
                <tr>
                    ...
                    <td>
                        @if($item->image)
                            <img src="{{ asset($item->image) }}" width="40" height="40" class="rounded me-2">
                        @else
                            <img src="{{ asset('assets/img/noimage.jpg') }}" width="40" height="40" class="rounded me-2">
                        @endif
                        {{ $item->judul }}
                    </td>
                    ...
                    <td>
                        <div class="btn-group">
                            ...
                            <ul class="dropdown-menu">
                                ...
                                @if($item->file)
                                <li><a class="dropdown-item" href="{{ asset($item->file) }}" target="_blank"><i data-feather="download"></i> Unduh File</a></li>
                                @endif
                                <li><a class="dropdown-item" href="javascript:void(0);" onclick="btnDeleteItem('{{ route($prefixRoute.'delete', $item->id) }}', '{{ $item->judul }}')"><i data-feather="trash"></i> Hapus</a></li>
                            </ul>
                        </div>
                    </td>
                </tr>
...
@include('admin.master.ruang-unduh._modal', ['prefixRoute' => $prefixRoute, 'title' => $title, 'kategoris' => $kategoris])
@endsection
@push('js')
    @include('admin.master.ruang-unduh._scripts', ['prefixRoute' => $prefixRoute])
@endpush
